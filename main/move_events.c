...
void mov_event2string(T_MOV_EVENT *evt, char *buf, size_t sz_buf) {
	switch(evt->type) {
	case MOV_EVENT_NOTHING: // no move
		snprintf(buf, sz_buf, "'nothing' start=%d",evt->start);
		break;
...
static esp_err_t process_effect_no_move(T_EVENT *evt, uint64_t timer_period) {
...
static esp_err_t process_effect_rotate(T_EVENT *evt, uint64_t timer_period) {
	...
	if ( mevt->w_t >= mevt->dt) {
		...
		mevt->w_t = 0;
		evt->isdirty = 1;
		mevt->w_pos += 1;  // TODO -1 for other direction
...
esp_err_t process_move_events(T_EVENT *evt, uint64_t timer_period) {
	...
	case MOV_EVENT_NOTHING: // no move
		return process_effect_no_move(evt, timer_period);
		break;
	case MOV_EVENT_ROTATE:
